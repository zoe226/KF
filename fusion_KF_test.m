%{
    there are two sets of observation, and merge them use KF
%}
...
for i = 1:L
    x(i) = t(i)^2;
    y1(i) = x(i) + normrnd(0,0.1); % Normal distribution noise
    y2(i) = x(i) + normrnd(0,0.01); % two sets of observation
end
% figure;plot(t,x,t,y1,t,y2,'LineWidth',2);
y = [y1;y2];
 %% filter
 ...
 H1 = [1,1]';
 Q1 = 1;
 R1 = [1,0;0.01,0]; % these initial value can be change
 % initial x(k)+
 Xplus1 = zeros(1,L);
...
%  figure;plot(t,x,'g',t,Xplus1,'r');
 ...
 for i = 2:L
  ...
  figure;plot(t,x,'g',t,Xplus1,'r',t,Xplus2(1,:),'k');
